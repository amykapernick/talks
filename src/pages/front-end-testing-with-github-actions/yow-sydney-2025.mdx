...
import Pa11y from '@img/a11y/pa11y.png'
...
<Slide styles="aoc">
	We acknowledge the traditional custodians of this land, the <mark>Gadigal</mark> people.
	<Notes slot="notes">
		- Travelled from home of the Whadjuk people
		- To land of the Bunurong (Boon Wurrung), not far from Wakka Wakka where I grew up
		- Then up to the land of the Jagera (Yuggera)
		- And lastly here on the land of the Gadigal people here with you all today
	</Notes>
</Slide>
...
<Slide>
	Why test?
	<Notes slot="notes">
		- First off, I'm not going to delve into why testing is important
		- If you're here I'm assuming it's a priority
		- There's an entire development process centred around the writing of tests, whether we should test has never been a question
	</Notes>
</Slide>
...
<Slide>
	...
	<Notes slot="notes">
		- But despite all these strong opinions
		- We often don't hear about testing on the front end
	</Notes>
</Slide>
...
<Slide>
	...
	<Notes slot="notes">
		- There are so may different tests that we need to be running on the front end
	</Notes>
</Slide>
...
<Slide 
	...
	>
		<Notes slot="notes">
			- There are so many things we need to test for, it's often hard to work out what you really need and where to start
		</Notes>
	</Slide>
...
<Slide>
	...
	<Notes slot="notes">
		- When testing the front end though, we need a live website (other than linting and HTML validation)
...
<Slide>
	Workflow <span class="fragment">→ Jobs</span> <span class="fragment">→ Steps</span>
	<Notes slot="notes">
...
<Slide type="code" size="xs" lines={['1,6', '2,7,12', '11', '8-9', '3-4,13-17']}>
	```yaml
	- name: Setup Node
	  uses: actions/setup-node@v3
	  with:
	...
	<Notes slot="notes">
		...
		4. Rather than running a step every time an action is triggered, we can set a conditional step that only runs when the condition is met
		5. We can pass in env variables for the script to use, most of the time likely referencing github secret values
...
<Slide type="code" lines={['1-2', '4-7', '9-10']} size="xs">
	```yaml
	- name: Checkout Repo Code
	  uses: actions/checkout@v3
...
<Slide type="code" size="xs" lines={['3', '5', '7']}>
	```bash
	#!/bin/bash
...
<Slide type="code" size="xs" lines={['1-6', '8-16', '2,15']}>
	```yaml
...
<Slide type="code" size="xs" lines={['1-6', '8-17']}>
	```yaml
	- name: Deploy to Netlify
	  id: build_site
	  env:
...
<Slide type="code" size="xs" lines={['8-17']}>
	```yaml
	- name: Deploy to Netlify
	  id: build_site
	  env:
...
<Slide type="code" size="xs" lines={['1-2,9', '2-3,9-10',  '12-14', '2,11', '4-5']}>
	```yaml
	jobs:
	  build:
		runs-on: ubuntu-22.04
...
<Slide type="code" size="small" lines={['4', '5-7']}>
	```yaml
	- name: Checkout Repo Code
	  uses: actions/checkout@v3
...
<Slide type="code" size="small" lines={['1-5', '7-12', '2,9-11', '9,12']}>
	```yaml
	- name: Decode URL
	  id: decode_url
	  run: |
...
<Slide type="code" size="xs" lines={['4-9', '11-16']}>
	```yaml
	- name: Run Playwright tests
	  run: npx playwright test
...
>
	<Notes slot="notes">
		- Easy to use command line tool, uses puppeteer for their headless browser
		- Does good a11y testing (although some needs doing manually)
		- Can integrate with a dashboard so other people can view and track data and issues
	</Notes>
</Slide>
...
<Slide type="code" size="small" lines={['4']}>
	...
	```
	<Notes slot="notes">
		- Using their CI specific tool, but the regular package would work as well, this is just setup a little nicer for headless browsers
		- Any CLI testing tool that can run in the terminal on your specified environment should be able to be used here
	</Notes>
</Slide>
...
<Slide type="code" size="small" lines={['1', '2-5', '6', '8-13']}>
	```yaml
	  - uses: actions/checkout@v3
	  - name: Setup Node
		uses: actions/setup-node@v3
...
<Slide>
	...
	<Notes slot="notes">
		- A bit tricky
		- Can be done, but requires a fair bit of workaround with this kind of pipeline Setup
		- Very specific to the individual use case, so not going into it here
	</Notes>
</Slide>
...
<Slide>
	<span class="fragment">
		- Build and Preview (anywhere) <span class="fragment">✅</span>
	</span>
	<span class="fragment">
		- Test live website <span class="fragment">✅</span>
	</span>
	<span class="fragment">
		- Build and Publish <span class="fragment">✅</span>
	</span>
	<Notes slot="notes">
		- 
	</Notes>
...
<Slide type="code" size="small" lines={['7-8', '1-2']}>
	```yaml
...
<Slide type="code" size="small" lines={['3-5']}>
	```yaml
	- name: Add Issues from Comments
	  uses: "alstr/todo-to-issue-action@v4"
	  with:
...
<Slide size="small">
	...
	<Notes slot="notes">
		...
		- Manual approval could be from QA, or a testing team to ensure that things have been checked and tested manually before the code can be merged
		- You could add a delay/check so it won't allow a PR to be merged to production between 4pm Thursday and 10am Monday
